/* zlib.d: modified from zlib.h by Walter Bright,
   modified again from zlib.h by Anders F Bjorklund */
...
alias Byte   Bytef;  /* Byte FAR */
alias uint    uInt;  /* 16 bits or more */
alias uInt   uIntf;  /* uInt FAR */
alias uint   uLong;  /* 32 bits or more */
alias uLong uLongf;  /* uLong FAR */

alias void* voidpc;  /* const void pointer */
alias void*  voidp;  /* void pointer */
alias voidp voidpf;  /* void FAR pointer */
...
const int MAX_MEM_LEVEL = 9; /* !defined(MAXSEG_64K) */
...
/* The memory requirements for deflate are (in bytes):
 ...
 that is: 128K for windowBits=15  +  256K for memLevel = 9  (default values)
...
alias voidpf (*alloc_func) (voidpf opaque, uInt items, uInt size);
alias void   (*free_func)  (voidpf opaque, voidpf address);
...
const uint Z_NO_FLUSH      = 0;
const uint Z_PARTIAL_FLUSH = 1; /* will be removed, use Z_SYNC_FLUSH instead */
const uint Z_SYNC_FLUSH    = 2;
const uint Z_FULL_FLUSH    = 3;
const uint Z_FINISH        = 4;
const uint Z_BLOCK         = 5;
/* Allowed flush values; see deflate() and inflate() below for details */

const uint Z_OK            = 0;
const uint Z_STREAM_END    = 1;
const uint Z_NEED_DICT     = 2;
const int Z_ERRNO        = -1;
...
const uint Z_FILTERED            = 1;
const uint Z_HUFFMAN_ONLY        = 2;
const uint Z_RLE                 = 3;
const uint Z_FIXED               = 4;
const uint Z_DEFAULT_STRATEGY    = 0;
/* compression strategy; see deflateInit2() below for details */

const uint Z_BINARY   = 0;
const uint Z_TEXT     = 1;
alias Z_TEXT      Z_ASCII;  /* for compatibility with 1.2.2 and earlier */
const uint Z_UNKNOWN  = 2;
/* Possible values of the data_type field (though see inflate()) */

const uint Z_DEFLATED   = 8;
/* The deflate compression method (the only one supported in this version) */

const voidpf Z_NULL  = null;  /* for initializing zalloc, zfree, opaque */
...
int inflateBack(z_stream* strm,
                in_func f_in, void* in_desc,
                out_func f_out, void* out_desc);
/*
...
int gzread(gzFile file, voidp buf, uint ulen);
/*
...
int deflateInit2_(z_streamp strm, int level, int method,
                  ...
                  int strategy, char* versionx,
                  int stream_size);
int inflateInit2_(z_streamp strm, int windowBits,
                  char* versionx, int stream_size);
int inflateBackInit_(z_stream* strm, int windowBits,
                     ubyte* window,
                     char* z_version,
                     int stream_size);
