...
alias std.c.stdlib.atoi atoi;
alias std.c.stdlib.atof atof;

const char[] LOGO_FILE = "icon.bmp";
...
private int power_of_two(int input)
{
	int value = 1;
...
GLuint SDL_GL_LoadTexture(SDL_Surface *surface, GLfloat *texcoord)
{
	...
	texcoord[2] = cast(GLfloat)surface.w / w;	/* Max X */
	texcoord[3] = cast(GLfloat)surface.h / h;	/* Max Y */
...
void DrawLogoTexture()
{
	...
	if ( ! texture ) {
		...
		/* Set color key to white color, RGB(#FFFFFF) */
...
void DrawLogoBlit()
{
	...
	//	temp = SDL_LoadBMP(LOGO_FILE);
		temp = SDL_LoadBMP_RW(SDL_RWFromFile(LOGO_FILE, "rb"), 1);
		if ( temp == null ) {
			return;
		}
		w = temp.w;
